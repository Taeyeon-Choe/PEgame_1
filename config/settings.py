...
from utils.constants import (
    ...
    PLOT_PARAMS, PATHS, DEFAULT_ORBIT_PARAMS
)
...
class PathConfig:
    ...
    def create_directories(self):
        """필요한 디렉토리 생성"""
        for path in [self.logs, self.models, self.results, 
                    self.tensorboard, self.checkpoints, self.plots, self.tests]:
...
class ProjectConfig:
    ...
    def __post_init__(self):
        ...
        if self.debug_mode:
            ...
            self.training.nash_total_timesteps = min(5000, self.training.nash_total_timesteps)
...
def get_config(experiment_name: Optional[str] = None, 
               debug_mode: bool = False,
               custom_config: Optional[Dict[str, Any]] = None) -> ProjectConfig:
    """설정 인스턴스 생성 및 반환"""
