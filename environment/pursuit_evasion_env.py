...
class PursuitEvasionEnv(gym.Env):
    ...
    def _normalize_obs(self, state: np.ndarray, pursuer_action: np.ndarray) -> np.ndarray:
        """관측값 정규화"""
    ...
    def initialize_with_accurate_dynamics(self) -> np.ndarray:
        ...
        for _ in range(100):
            ...
            rel = convert_orbital_elements_to_relative_state(
                a_e, e_e, i_e, RAAN_e, omega_e, M_e,
                a_p, e_p, i_p, RAAN_p, omega_p, M_p
            )
    ...
    def reset(self) -> np.ndarray:
        ...
        self.evader_orbit = ChiefOrbit(
            a=orbit_config.a, e=orbit_config.e, i=orbit_config.i,
            RAAN=orbit_config.RAAN, omega=orbit_config.omega, M0=orbit_config.M0,
            mu=MU_EARTH
        )
    ...
    def get_optimal_interception_directions(self, rho: np.ndarray, v_rel: np.ndarray, 
                                          prediction_time: float) -> list:
        """최적 방향 세트 계산"""
        # 1. 직접 추격
        ...
        if len(self.pursuer_success_history) > 0 and np.random.random() < 0.3:
            ...
            for i, (old_rho, old_success) in enumerate(self.pursuer_success_history):
                if old_success and np.linalg.norm(rho - old_rho) < self.capture_distance * 3:
    ...
    def _apply_evader_delta_v(self, delta_v_e: np.ndarray):
        ...
        # 회피자 궤도 업데이트
        self.evader_orbit = ChiefOrbit(
            a=a_e_new, e=e_e_new, i=i_e_new,
            RAAN=RAAN_e_new, omega=omega_e_new, M0=M_e_new,
            mu=MU_EARTH
        )
        
        # 새 LVLH에서 추격자 상대 상태 계산
        a_p, e_p, i_p, RAAN_p, omega_p, M_p = state_to_orbital_elements(r_pursuer, v_pursuer)
    ...
    def check_time_buffered_termination(self) -> Tuple[bool, Dict]:
        ...
        if (len(self.capture_status_history) >= self.capture_buffer_steps and 
            all(self.capture_status_history)):
            self.termination_details = {
                'outcome': 'captured',
                'evader_reward': -10,
                'pursuer_reward': 10,
        ...
        if rho_mag > self.evasion_distance:
            ...
            if (len(self.evasion_status_history) >= self.evasion_buffer_steps and
                all(self.evasion_status_history)):
    ...
    def _calculate_rewards(self, done: bool, termination_info: Dict, 
                         delta_v_e_mag: float) -> Tuple[float, float, Dict]:
        """보상 계산"""
        if done:
            ...
            evader_reward = distance_reward + tangential_reward - control_cost + dodge_bonus
    ...
    def analyze_results(self, states: np.ndarray, actions_e: np.ndarray, 
                       actions_p: np.ndarray) -> Dict:
        """결과 분석"""
        final_distance = np.linalg.norm(states[-1, :3])
        total_e_delta_v = np.sum([np.linalg.norm(a) for a in actions_e])
        total_p_delta_v = np.sum([np.linalg.norm(a) for a in actions_p if np.any(a != 0)])
