...
def main():
    ...
    # 1. 설정 로드
    config = get_config(experiment_name="basic_example", debug_mode=False)
    ...
    # 2. 환경 생성 (멀티프로세싱)
    env_num = config.training.n_envs if not config.debug_mode else 1

    def make_env():
        return PursuitEvasionEnv(config)

    env = SubprocVecEnv([make_env for _ in range(env_num)])
    print(f"환경 생성 완료 - {env_num}개 병렬")
