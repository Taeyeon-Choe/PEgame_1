...
def relative_dynamics_evader_centered(t: float, state: List[float], evader_orbit) -> List[float]:
    """
    ...
    # 기본 상대 운동 방정식 (비선형)
    ddot_x = (2 * dot_theta0 * vy + ddot_theta0 * y + dot_theta0**2 * x -
              MU_EARTH * (r0 + x) / rho_norm**3 + MU_EARTH / r0**2)
    ddot_y = (-2 * dot_theta0 * vx - ddot_theta0 * x + dot_theta0**2 * y -
...
def compute_j2_differential_acceleration(t: float, state: List[float], evader_orbit) -> np.ndarray:
    """
    J2 섭동의 차등 가속도 계산
...
def atmospheric_drag_acceleration(r_eci: np.ndarray, v_eci: np.ndarray, 
                                cd_area_mass: float = 1e-3) -> np.ndarray:
    """
    대기 항력 가속도 계산 (간단한 모델)
...
def solar_radiation_pressure(r_eci: np.ndarray, r_sun_eci: np.ndarray,
                           cr_area_mass: float = 1e-3) -> np.ndarray:
    """
    태양 복사압 가속도 계산
...
def third_body_acceleration(r_eci: np.ndarray, r_body_eci: np.ndarray, 
