...
class ChiefOrbit:
    ...
    def __init__(self, a: float, e: float, i: float, RAAN: float, 
                 omega: float, M0: float, mu: float = MU_EARTH):
        """
        궤도 요소로 궤도 초기화
    ...
    def _solve_kepler_newton_raphson(self, M: float, E0: float, 
                                   max_iter: int = 20, tol: float = 1e-12) -> float:
    ...
    def copy(self) -> 'ChiefOrbit':
        """궤도 복사본 생성"""
        return ChiefOrbit(
            self.a, self.e, self.i, self.RAAN, 
            self.omega, self.M0, self.mu
        )
