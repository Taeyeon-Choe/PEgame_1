...
class SACTrainer:
    ...
    def setup_callbacks(self, custom_callbacks: Optional[List] = None):
        ...
        evasion_callback = EvasionTrackingCallback(
            verbose=1, log_dir=f"{self.log_dir}/plots"
        )
        ...
        # 3. 성능 모니터링
        performance_callback = PerformanceCallback(eval_freq=1000, verbose=0)
        ...
        # 5. 조기 종료 (선택적)
        if (
            hasattr(self.training_config, "early_stopping")
            and self.training_config.early_stopping
        ):
            early_stopping_callback = EarlyStoppingCallback(
                target_success_rate=0.8, patience=10000, verbose=1
            )
    ...
    def evaluate(
        ...
        results = {"episode_rewards": [], "episode_lengths": [], "success_count": 0}
        ...
        for episode in range(n_episodes):
            ...
            print(
                f"에피소드 {episode+1}: 보상={episode_reward:.2f}, 길이={episode_length}"
            )

        # 평가 결과 요약
        results["mean_reward"] = sum(results["episode_rewards"]) / len(
            results["episode_rewards"]
        )
        results["std_reward"] = torch.tensor(results["episode_rewards"]).std().item()
        results["mean_length"] = sum(results["episode_lengths"]) / len(
        ...
        print(f"\n평가 결과:")
        print(
            f"  평균 보상: {results['mean_reward']:.2f} ± {results['std_reward']:.2f}"
        )
    ...
    def continue_training(
        self, additional_timesteps: int, tb_log_name: str = "sac_continue"
    ) -> "SACTrainer":
        """학습 계속하기"""
        if self.model is None:
            print("계속할 모델이 없습니다.")
    ...
    def fine_tune(
        self, new_env, fine_tune_timesteps: int, learning_rate_multiplier: float = 0.1
    ) -> "SACTrainer":
        """파인 튜닝"""
        if self.model is None:
...
def create_trainer(
    env, config: Optional[ProjectConfig] = None, experiment_name: Optional[str] = None
) -> SACTrainer:
    """트레이너 생성 헬퍼 함수"""
    if config is None:
